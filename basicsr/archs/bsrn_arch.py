...
@ARCH_REGISTRY.register()
class BSRND(nn.Module):
    def __init__(self, num_in_ch=3, num_feat=64, num_block=8, num_out_ch=3, upscale=4,
                 conv='BSConvU', upsampler='pixelshuffledirect', p=0.25):
        super(BSRND, self).__init__()
        kwargs = {'padding': 1}
        if conv == 'BSConvS':
            kwargs = {'p': p}
        print(conv)
        if conv == 'DepthWiseConv':
            ...
            self.conv = nn.Conv2d
        self.fea_conv = self.conv(num_in_ch * 4, num_feat, kernel_size=3, stride=2, **kwargs)
        self.input_conv = self.conv(num_in_ch * 4, num_feat, kernel_size=3, stride=1, **kwargs)
        self.B1 = ESDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B2 = ESDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B3 = ESDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        self.B4 = ESDB(in_channels=num_feat, out_channels=num_feat, conv=self.conv, p=p)
        ...
        self.c2 = self.conv(num_feat, num_feat, kernel_size=3, **kwargs)
        self.merge_conv = self.conv(num_feat, num_feat, kernel_size=3, **kwargs)
        if upsampler == 'pixelshuffledirect':
            self.upsampler1 = PixelShuffleDirect(scale=2, num_feat=num_feat, num_out_ch=num_feat)
            self.upsampler2 = PixelShuffleDirect(scale=upscale, num_feat=num_feat, num_out_ch=num_out_ch)
        else:
            raise NotImplementedError(("Check the Upsampeler. None or not support yet"))
    ...
    def forward(self, input):
        ...
        out_lr = self.upsampler1(self.c2(out_B) + out_fea)
        output = self.upsampler2(self.merge_conv(out_lr+self.input_conv(input)))

        return output
if __name__ == '__main__':
    import torchstat
    model = BSRND(upscale=2)
    torchstat.stat(model, (3,256,256))
